                            ...
                            Ok(listener1) => {
                                ...
                                for stream in listener1.incoming() {
                                    ...
                                    for i in 1..fin_by_client {
                                        stream.read(&mut buffer[..]).expect(&format!("cannot read from stream at try {}", i+1));
                                        stream.write(&format!("Thank you, {} times", i+1).as_bytes()).expect("cannot write to stream");
                                    }
                                }
                            }
                            _ => {
