...
use read_config;
use {get_mac_from_ifname, print_hard_statistics};
use setup_pipelines;
use L234Data;
...
pub fn run_test(test_type: TestType) {
        ...
        .and_then(|ctxt| check_system(ctxt))
    {
        Ok(mut context) => {
            ...
            // retrieve a port mask:
            // let port_mask=context.ports.values().last().unwrap().get_tcp_dst_port_mask();
            let port_mask=u16::from_be(netbricks_configuration.ports[0].fdir_conf.unwrap().mask.dst_port_mask);

            let port_count = context.ports.len() as u16;
            debug!("port count = { }, port mask = 0x{:x}", port_count, port_mask);
            ...
            if test_type == TestType::Server {
                ...
                for ntry in 0..configuration.test_size.unwrap() as u16 {
                    let target_port=0xFFFF-(!port_mask+1)*(ntry%port_count);
                ...
                for (p, c_records) in &con_records {
                    ...
                    c_records.iter().enumerate().for_each(|(i, c)| {
                        ...
                        if c.get_release_cause() == ReleaseCause::FinServer
                            && c.s_states().last().unwrap() == &TcpState::Closed
                            {
                                completed_count += 1
                            };
                    });
                    assert_eq!(completed_count, tcp_counters_from.get(&p).unwrap()[TcpControls::RecvSyn]);
                    assert_eq!(tcp_counters_from.get(&p).unwrap()[TcpControls::RecvSyn], tcp_counters_from.get(&p).unwrap()[TcpControls::SentSynAck]);
                    assert_eq!(tcp_counters_from.get(&p).unwrap()[TcpControls::SentSynAck], tcp_counters_from.get(&p).unwrap()[TcpControls::RecvSynAck2]);
                    assert_eq!(tcp_counters_from.get(&p).unwrap()[TcpControls::RecvFin], tcp_counters_from.get(&p).unwrap()[TcpControls::SentFinAck]);
                    assert_eq!(tcp_counters_from.get(&p).unwrap()[TcpControls::SentFinAck], tcp_counters_from.get(&p).unwrap()[TcpControls::RecvFinAck2]);
                }
            }
            if test_type == TestType::Client {
                for (p,c_records) in &con_records {
